...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  let defaultTimezone = moment.tz.guess() || browserTimezone;
  let error = { error: `Error getting GeoData`, message: `Error getting GeoData` };
  let browserTimezoneCityOrRegion = defaultTimezone.split(`/`)[1].replace(/_/g, ` `);
  ...
  const getMapData = (coordinates) => {
    ...
    // let defaultZoom = `Region`;
    // let zoomLevel = mapZoomLevels[defaultZoom.toLowerCase()];
    // let mapSourceLink = `${mapsEmbedURL}?pb=${zoomLevel}!2d${longitude}!3d${latitude}${googleMapsEmbedOptions}`;
    // return mapSourceLink;
    ...
    //   mapSourceLink,
    //   mapZoomLevels,
    //   enableDefaultZoomCode,
    //   googleMapsEmbedOptions,
    //   urlStructure: '${mapsEmbedURL}?pb=${zoomLevel}!2d${longitude}!3d${latitude}${googleMapsEmbedOptions}',
    // };
  }
