...
export const defaultSetParameters = {
    Default: {
        id: 0,
        min: 1,
        max: 9,
...
export default function CommandsForm(props) {
    ...
    let [activeSetParameter, setActiveSetParameter] = useState<any>(defaultSetParameters.Default);
    ...
    let [setParameters, setSetParameters] = useState<any>([
        defaultSetParameters.Level,
        defaultSetParameters.Experience,
        defaultSetParameters.XP_Modifier,
    ]);
    ...
    const adjustCommand = (e, val) => {
        if (val) {
            ...
            setActiveSetParameter(defaultSetParameter);
    ...
    const renderCommand = (command: Command) => {
        ...
        if (command) {
            if (command.command == defaultCommands.Delete.command) {
                commandToReturn = `!del ${playersSelected}`;
            } else if (command.command == defaultCommands.Set.command) {
                commandToReturn = `!set ${playersSelected} ${setParameter} ${amount.toLocaleString()}`;
            } else if (command.command == defaultCommands.Give.command) {
            ...
            <ul className={`commandsList commandToCopy ${command?.name}`}>
                <li className={`listedCommand`} title={renderCommand(command)}>
                    <div className="commandDetails flex gap15">
                        <CodeBlock commandToCopy={true} custombutton={true} border={`solid 2px white`} id={`commandToRender`} className={`commandToRender ${defaultSetParameters.XP_Modifier.shortcuts.includes(setParameter.toLowerCase()) && (amount != defaultAmount) && (setParameter.toLowerCase() != defaultSetParameter.toLowerCase()) ? `hasContent` : `noExtraContent`}`} language={`js`} codeTitle={<>
                            {command?.icon}
            ...
            <section className={`formsSection`}>
                <form onSubmit={(e) => submitCommandsForm(e, user)} className={`commandsForm ${command.class} ${defaultCommands.Update.command == command.command ? `updateCommandForm` : `customHookInputContainer`} ${dynamicFormCommands.includes(command.command) ? `dynamicHeight` : `staticHeight`} gridForm ${iPhone ? `on_iPhoneCommandForm` : `notOn_iPhoneCommandForm`}`} action={`submit`}>
                    ...
                    <div className={`deleteCommandsForm multiSelectCommandsForm commandInputs ${command.command == `!del` ? `expanded` : `collapsed`}`}>
                        <PlayerSelector id={`deletePlayersSelector`} />
                    </div>
                    <div className={`setCommandsForm multiSelectCommandsForm commandInputs ${command.command == `!set` ? `expanded` : `collapsed`}`}>
                        ...
                        <div className={`updateRow nextParameterRow updateNextRow ${(amount != defaultAmount) && (setParameter.toLowerCase() != defaultSetParameter.toLowerCase()) ? `nextInitialized` : `nextNotSet`} setParameterRangeRow ${setParameter.toLowerCase() != defaultSetParameter.toLowerCase() ? `expanded` : `collapsed`}`}>
                            {setParameter == defaultSetParameters.Level.label ? <>
                                <RangeSlider 
                                    min={defaultSetParameters.Level.min} 
                                    max={defaultSetParameters.Level.max} 
                                    name={`Set to ${defaultSetParameters.Level.label}`} 
                                    defaultValue={defaultSetParameters.Level.defaultValue} 
                                />
                            </> : <></>}
                            {setParameter == defaultSetParameters.Experience.label ? <>
                                <RangeSlider 
                                    min={defaultSetParameters.Experience.min} 
                                    max={defaultSetParameters.Experience.max} 
                                    name={`Set ${defaultSetParameters.Experience.label} to`} 
                                    defaultValue={defaultSetParameters.Experience.defaultValue} 
                                />
                            </> : <></>}
                            {setParameter == defaultSetParameters.XP_Modifier.label ? <>
                                <RangeSlider 
                                    min={defaultSetParameters.XP_Modifier.min} 
                                    max={defaultSetParameters.XP_Modifier.max} 
                                    name={`Set ${defaultSetParameters.XP_Modifier.label} to`} 
                                    defaultValue={defaultSetParameters.XP_Modifier.defaultValue}
                                    contextText={(amount != defaultAmount) && (setParameter.toLowerCase() != defaultSetParameter.toLowerCase()) ? `x Multiplier` : ``} 
                                />
                            </> : <></>}
                        </div>
                        <div className={`updateRow nextParameterRow fixedPositioning updateNextRow ${(amount != defaultAmount) && (setParameter.toLowerCase() != defaultSetParameter.toLowerCase()) ? `expanded` : `collapsed`}`}>
                            <PlayerSelector id={`setPlayersSelector`} />
                        </div>
                    </div>
                    <div className={`giveCommandsForm multiSelectCommandsForm commandInputs ${command.command == `!giv` ? `expanded` : `collapsed`}`}>
                        <PlayerSelector id={`givePlayersSelector`} />
                    </div>
